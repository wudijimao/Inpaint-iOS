...
//    import VideoToolbox
    ...
    public func buffer(ofSize size: Int) -> CVPixelBuffer?  {
    ...
    public func grayBuffer(ofSize size: Int) -> CVPixelBuffer?  {
        let feature = try? MLFeatureValue.init(cgImage: self.cgImage!, pixelsWide: size, pixelsHigh: size, pixelFormatType: kCVPixelFormatType_OneComponent8)
        return feature?.imageBufferValue
    }
...
extension UIImage {
    ...
    public func scaleToLimit(size limitSize: CGSize) -> UIImage {
